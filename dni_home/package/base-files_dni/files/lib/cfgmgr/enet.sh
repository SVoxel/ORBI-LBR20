...
sw_print_ssdk_cmds_add_vlan_for_wan() # $1: vid
{
	cat <<EOF
$ssdk_sh vlan entry create $1
$ssdk_sh vlan entry append $1 $1 0,4 0,4 null null no no
EOF
}
...
sw_print_ssdk_cmds_add_vlan_for_wan_with_untag() # $1: vid
{
	cat <<EOF
$ssdk_sh vlan entry create $1
$ssdk_sh vlan entry append $1 $1 0,4 0 4 null no no
$ssdk_sh portvlan defaultcvid set 1 $1
EOF
}
...
sw_configvlan_factory()
{
	...
	if [ "x$($CONFIG get factory_tt3)" = "x1" ]; then
		sw_printconf_add_vlan "switch0" "1" "1" "4 5" >> $swconf
		sw_printconf_add_vlan "switch0" "2" "2" "0" >> $swconf
	else
		sw_printconf_add_vlan "switch0" "1" "1" "0 4 5" >> $swconf
	fi
	$swconfig dev switch0 load $swconf
}
...
sw_configvlan_apmode()
{
	...
	if [ "x$arlo_enale" = "x1" ] ; then
		sw_printconf_add_vlan "switch0" "1" "1" "0t 5" >> $swconf
		sw_printconf_add_vlan "switch0" "2" "2" "0t 4" >> $swconf
		sw_printconf_add_vlan "switch0" "3" "arlo_vid" "0t 5t" >> $swconf
	else
		sw_printconf_add_vlan "switch0" "1" "1" "0 5" >> $swconf
		sw_printconf_add_vlan "switch0" "2" "2" "0 4" >> $swconf
	fi
	$swconfig dev switch0 load $swconf
}
	...
	if [ "x$($CONFIG get factory_tt3)" = "x1" ]; then
		...
		if [ "x$arlo_enale" = "x1" ] ; then
			if [ "$module_type" = "Base" ];then
				...
				sw_printconf_add_vlan "switch0" "1" "1" "0t 4 5" >> $swconf
				sw_printconf_add_vlan "switch0" "2" "$arlo_vid" "0t 4t 5t" >> $swconf
			fi
			$swconfig dev switch0 load $swconf
		else
			sw_printconf_add_vlan "switch0" "1" "1" "0 5" >> $swconf
...
{
	local i mask=$(($1 & 0x1))
	local ports1="0t" ports2="0t 4" ports3="0t" ports4="0t 4t"
	...
	for i in 5; do
		if [ $(( $(i_mask $i) & $mask )) -eq 0 ]; then
			ports1="$ports1 $i"
			ports3="$ports3 ${i}t"
		elif [ "$iptv_vlan_enable" = "1" ]; then
			ports4="$ports4 ${i}t"
		#fix bug #102049, port will belong to vlan and LAN at same time
			ports1="$ports1 $i"
		else
			ports2="$ports2 $i"
		fi
