...
# check items for assigned CPUs of Receive Packet Steering and
...
#Check smp affinity parameters of HW IRQ
printf " Checking smp affinity parameters\n"

if [ "x$SMP_AFFINITY_DNI" = "x1"  ]; then
    Irq_164_Smp_Affinity=`cat /proc/irq/$HW_IRQ_NUM_USB/smp_affinity`
    show_result "smp.affinity.164" "$Irq_164_Smp_Affinity" "$SMP_AFFINITY_USB"

    Irq_174_Smp_Affinity=`cat /proc/irq/174/smp_affinity`
    show_result "smp.affinity.174" "$Irq_174_Smp_Affinity" "$SMP_AFFINITY_WIFI2"
    
    Irq_200_Smp_Affinity=`cat /proc/irq/200/smp_affinity`
    show_result "smp.affinity.200" "$Irq_200_Smp_Affinity" "$SMP_AFFINITY_WIFI0"

    Irq_201_Smp_Affinity=`cat /proc/irq/201/smp_affinity`
    show_result "smp.affinity.201" "$Irq_201_Smp_Affinity" "$SMP_AFFINITY_WIFI1"

    #Check RPS parameters of SW IRQ
    printf " Checking RPS parameters\n"
    if [ -f "/sys/class/net/wwan0/queues/rx-0/rps_cpus" ]; then
        Queues_rps_cpus_wwan0=`cat /sys/class/net/wwan0/queues/rx-0/rps_cpus`
        show_result "queues.rps.cpus.wwan0" "$Queues_rps_cpus_wwan0" "$RPS_WWAN0"
    else
        printf "\tChecking %-55s %10s\n" "queues.rps.cpus.wwan0" "wwan0 not found"
    fi
    
    Queues_rps_cpus_2G_FH=`cat /sys/class/net/ath0/queues/rx-0/rps_cpus`
    show_result "queues.rps.cpus.2G.FH" "$Queues_rps_cpus_2G_FH" "$RPS_wifi0"

    Queues_rps_cpus_2G_BH=`cat /sys/class/net/ath01/queues/rx-0/rps_cpus`
    show_result "queues.rps.cpus.2G.BH" "$Queues_rps_cpus_2G_BH" "$RPS_wifi0"

    Queues_rps_cpus_5G_FH=`cat /sys/class/net/ath1/queues/rx-0/rps_cpus`
    show_result "queues.rps.cpus.5G.FH" "$Queues_rps_cpus_5G_FH" "$RPS_wifi1"

    Queues_rps_cpus_5G_BH=`cat /sys/class/net/ath11/queues/rx-0/rps_cpus`
    show_result "queues.rps.cpus.5G.BH" "$Queues_rps_cpus_5G_BH" "$RPS_wifi1"
else
    Irq_200_Smp_Affinity=`cat /proc/irq/200/smp_affinity`
