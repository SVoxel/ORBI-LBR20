...
int phddns_step(PHGlobal *phglobal)
{
	...
	if (phglobal->bNeed_connect)
	{
		...
		if (phglobal->cbOnStatusChanged) phglobal->cbOnStatusChanged(Connecting, -1);
		...
		{
			syslog(LOG_INFO, "[Dynamic DNS] host name %s registration successful",ipaddr );
			//OnUserInfo(phglobal->szUserInfo);
			//OnAccountDomainInfo(phglobal->szDomainInfo);
			if (phglobal->cbOnStatusChanged) phglobal->cbOnStatusChanged(Connecting, phglobal->nUserType);
		...
		else 
		{
			...
			{
				if (phglobal->cbOnStatusChanged) phglobal->cbOnStatusChanged(AuthFail, -1);
			...
			{
				if (phglobal->cbOnStatusChanged) phglobal->cbOnStatusChanged(Connecting, -1);
				LOG(1) ("ExecuteUpdate ConnectFailed, waiting for 5 seconds to retry...\n");
				return 5;
			}
			if(ret == errorDomainRegisterFailed)
				syslog(LOG_INFO, "[Dynamic DNS] host name %s registration failure,", ipaddr);
			if (phglobal->cbOnStatusChanged) phglobal->cbOnStatusChanged(RegFail, -1);
	...
	{
		if (phglobal->cbOnStatusChanged) phglobal->cbOnStatusChanged(Online, phglobal->nUserType);
