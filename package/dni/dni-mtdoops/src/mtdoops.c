...
#define MTD_DUMPPART  "vol_oopsdump"
...
extern unsigned long reboot_reason_flags;
...
static int dni_ubi_read_for_proc(char *msg)
{
	...
	mtd = get_mtd_device_nm(MTD_DUMPPART);
...
{
	char reboot_reason_flags_str[DNI_PAGE_SIZE]={0};
...
{
	char buffer[8], reboot_reason_flags_str[DNI_PAGE_SIZE]={0};
	int len = 4;
