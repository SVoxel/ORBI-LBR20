...
#define DEFAULT_SCRIPT2		"/usr/share/udhcpc/default.script.wwan"
#define DEFAULT_SCRIPT_AP2	"/usr/share/udhcpc/default.script.wwan.ap"
...
#define SIM_CABLE_FILE	"/tmp/mobile_output/sim_status"
...
static inline int sim_up(void)
{
	...
	fd = open(SIM_CABLE_FILE, O_RDONLY, 0666);
	if (fd < 0)
		return 0;

	if (read(fd, value, 8) >= 0 && strcmp(value, "UNINSERT"))
		link_up = 1;
	else
		link_up = 0;
...
#endif
{
	...
	if (!script_specified) {
		if (!strcmp(client_config.interface, "wwan0")) {
			if (client_config.apmode)
				client_config.script = DEFAULT_SCRIPT_AP2;
			else
				client_config.script = DEFAULT_SCRIPT2;
	...
	for (;;) {
		...
		if (listen_mode != LISTEN_NONE && fd < 0) {
			if (listen_mode == LISTEN_KERNEL)
				...
				fd = raw_socket_client(client_config.ifindex, CLIENT_PORT);
			if (fd < 0) {
		...
		if (retval == 0) {
			...
			case INIT_SELECTING:
				if (packet_num < (client_config.apmode ? (!br_mode_enable() ? 5 : 5 ) : 3)) {
					 ...
					 */
					if (!((eth_up() && strcmp(client_config.interface, "wwan0")) || (sim_up() && !strcmp(client_config.interface, "wwan0"))) && !client_config.apmode) {
