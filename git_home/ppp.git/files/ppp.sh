...
# 1.$user 2.$mtu 3.$idle 4.$demand 5.$ip 6.$dns 7.$gw 8.$WAN_IF 9.$test
print_l2tp_options(){
    cat <<EOF
noauth
noipdefault
...
if [ "\$(config get wan_proto)" = "pptp" -a "\$(config get wan_pptp_dns_assign)" = "1" ]; then
        if [ "x\$staticdns1" != "x" ]; then
			...
			if [ "x\$(config get pptp_gw_static_route)" != "x" ]; then
				echo nameserver "\$staticdns1" > \$RESOLV_CONF
		...
		if [ "x\$staticdns2" != "x" ]; then
			...
			if [ "x\$(config get pptp_gw_static_route)" != "x" ]; then
				echo nameserver "\$staticdns2" >> \$RESOLV_CONF
...
if [ "\$(config get wan_proto)" = "pptp" -a "\$(config get wan_pptp_wan_assign)" = "0" ]; then
	cat /tmp/dhcpc_resolv.conf >> \$RESOLV_CONF
...
#restart circle to fix lan device cannot ping through internet
...
echo "[DB] PPP failover-ip-up script is done" > /dev/console
...
print_failover_ip_down()
{
    cat <<EOF
#!/bin/sh
echo "[DB] PPP failover-ip-down script is done" > /dev/console
...
prepare_failover()
{
	print_failover_ip_up > /etc/ppp/failover-ip-up
	chmod 0777 /etc/ppp/failover-ip-up
	print_failover_ip_down > /etc/ppp/failover-ip-down
	chmod 0777 /etc/ppp/failover-ip-down
}

setup_interface_ppp() {
	local user passwd dns mtu mru idle demand service ip proto gw language staticdns1 staticdns2 pptp_wan_assign pppoe_type test
	...
	[ "x$wan_policy" = "xfailover" ] && prepare_failover

	proto=$($CONFIG get wan_proto)

	test=$($CONFIG get run_test)
	[ "x$test" = "xtest" ] || test=""
	...
	if [ "$proto" = "pptp" ]; then
		...
		if [ "$($CONFIG get wan_endis_dod)" = "1" ]; then
		    ...
		    [ "x$1" != "xmanually" -a "x$test" != "xtest" ] && exit 0
		    idle="runone"
		else
		    idle="persist"
		fi
		...
		print_pptp_options "$user" "${mtu:-1492}" "$idle" "$demand" "$ip" "$dns" "$gw" "$WAN_IF" "$test" > /etc/ppp/peers/dial-provider
        ...
        print_l2tp_options "$user" "${mtu:-1428}" "$idle" "$demand" "$ip" "$dns" "$gw" "$WAN_IF" "$test" > /etc/ppp/peers/dial-provider
		...
		if [ "x$($CONFIG get failover_wired_proto)" = "xorange_pppoe" ];then
			user=$($CONFIG get wan_orange_username)
			passwd=$($CONFIG get wan_orange_pppoe_passwd)
		else
        ...
        if [ "x$pppoe_type" = "x1" -a "x$ipv6_wantype" = "xpppoe" ] ;then
               print_pppoe_options_v6 "$user" "$dns" "${mtu:-1492}" "$idle" "$demand" "$service" "$ip" "$WAN_IF" "$test" > /etc/ppp/peers/dial-provider
        else
               print_pppoe_options "$user" "$dns" "${mtu:-1492}" "$idle" "$demand" "$service" "$ip" "$WAN_IF" "$test" > /etc/ppp/peers/dial-provider
        fi
    ...
    user=$(echo ${user} | sed 's/\\/\\\\/g' | sed 's/\#/\\#/g' | sed 's/"/\\"/g' | sed "s/'/\\\'/g")
    passwd=$(echo ${passwd} | sed 's/\\/\\\\/g' | sed 's/\#/\\#/g' | sed 's/"/\\"/g' | sed "s/'/\\\'/g")
