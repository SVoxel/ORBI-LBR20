...
#define SIM_STATUS 	"/tmp/mobile_output/sim_status"
...
/*It is ONLY used for SIM Card. Only 'UNINSERT' means SIM Card Not UP.*/
static inline int sim_up(void)
{
	return readc(SIM_STATUS) != 'U';
}
...
static int net_verified(void)
{
	...
	if (config_match("wan_type", "mobile"))
		alive = sim_up();
	else {
...
int primary_loop(int usd, struct ntp_control *ntpc)
{
	fd_set fds;
...
int main(int argc, char *argv[]) {
	...
	if(wan_proto[0] == '\0') {
		if(config_match("wan_type", "mobile"))
			strncpy(wan_proto, config_get("wan2_proto"), sizeof(wan_proto)-1);
		else
	...
	/*When user manually enters the preferred NTP server, the preferred NTP server shall be set as primary time server and  secondary host time server is the same ,
	 */
...
//	config_commit();
	 ...
	 * */
		 ...
		 * reasons, the retries shall iterate swithching between primart
		 * and secondary server, and between 123 and non-123 port
