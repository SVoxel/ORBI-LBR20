...
static int scan_rootfs_header(struct mtd_info *master, struct mtd_info *mtd, uint64_t *offset, int *bad_blk_count)
{
	...
	while (*offset < mtd->size) {
		...
		if (!mtd_check_rootfs_magic(mtd, (size_t)*offset)) {
...
static void remap_blocks_in_rootfs_partition(struct mtd_part *slave)
{
	...
	if (slave->mtd.name &&
		(!strcmp(slave->mtd.name, NAME_OF_MAPPING_PARTITION_1) || !strcmp(slave->mtd.name, NAME_OF_MAPPING_PARTITION_2)) &&
 ...
 */
#ifdef SUPPORT_DNI_DUAL_FIRMWARE
...
#define DNI_BOARD_DATA2_PARTITION_OFFSET 0x00e00000	/* must same as dni_nand_partition in ofpart.c */

int boot_from_dni_dual_firmware(struct mtd_info *master, int rootfs1_index, int rootfs2_index)
{
	char boot_index;
	int ret_len;
	unsigned int rootfs_mtd_index;

	mtd_read(master, DNI_BOARD_DATA2_PARTITION_OFFSET, sizeof(boot_index),
			&ret_len, (void *)&boot_index);
	...
	if (boot_index == 0x31)
		rootfs_mtd_index = rootfs1_index;
	else
		rootfs_mtd_index = rootfs2_index;

	printk(KERN_NOTICE "%s: booting rootfs from mtd%d\n", __func__, rootfs_mtd_index);
	ROOT_DEV = MKDEV(MTD_BLOCK_MAJOR, rootfs_mtd_index);
		       ...
		       int nbparts)
{
	...
	int i;
	int rootfs1_index = -1, rootfs2_index = -1;
...
#ifdef SUPPORT_DNI_DUAL_FIRMWARE
		if (slave->mtd.name) {
			if (!strcmp(slave->mtd.name, NAME_OF_MAPPING_PARTITION_1))
				rootfs1_index = slave->mtd.index;
			else if (!strcmp(slave->mtd.name, NAME_OF_MAPPING_PARTITION_2))
				rootfs2_index = slave->mtd.index;
		}
#endif
...
#ifdef SUPPORT_DNI_DUAL_FIRMWARE
	if (rootfs1_index < 0 || rootfs2_index < 0)
		printk(KERN_ALERT "%s: do not found rootfs1 or rootfs2 partition!!!\n", __func__);
	else
		boot_from_dni_dual_firmware(master, rootfs1_index, rootfs2_index);
#endif
