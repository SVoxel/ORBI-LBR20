...
static void usb_wwan_indat_callback(struct urb *urb)
{
	...
	if(urb->dev->descriptor.idVendor == cpu_to_le16(0x2C7C) || urb->dev->descriptor.idVendor == cpu_to_le16(0x05C6)) {
		if(urb->status == -EPROTO) {
			dev_err(dev, "%s: nonzero status: %d on endpoint %02x.\n", __func__, urb->status, endpoint);
			return;
		}
	}
