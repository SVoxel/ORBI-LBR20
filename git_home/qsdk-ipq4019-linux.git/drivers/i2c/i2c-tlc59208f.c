...
/*M7533*/
#define SENSOR_CONF             0x09
#define SENSOR_ALERT		0xBF
...
EXPORT_SYMBOL_GPL(tlc59208f_read);
...
EXPORT_SYMBOL_GPL(tlc59208f_write);
...
int tlc59208f_reg_init(char mode1, char mode2, char out0, char out1)
{
	...
	//sensor_write(I2C_BYTE, SENSOR_TOS, 0x50, 1);
	//sensor_write(I2C_BYTE, SENSOR_CONF, 0x01, 1); // Conf normal mode
	printk(KERN_ERR " Init Sensor temp = %04x, shutdown temp = %04x\n",i2c_smbus_read_byte_data(g_client_s, 0x0), i2c_smbus_read_byte_data(g_client_s, 0x5));
	}
#endif
}
