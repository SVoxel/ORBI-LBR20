 ...
 */
extern int mmc_dni_art_read_boot_part(char *part_no);
static void read_boot_part_from_art(struct parsed_partitions *state)
{
	...
	mmc_dni_art_read_boot_part(boot_part_num);
	pr_notice("%s: the rootfs part num is #%s\n", __func__, boot_part_num);

	if (strncmp(boot_part_num, "02", 2) == 0)
		root_part = 24; /* boot from the second rootfs */
	else
		root_part = 20;

	if (ROOT_DEV == 0 && config_enabled(CONFIG_MTD_ROOTFS_ROOT_DEV)) {
...
int efi_partition(struct parsed_partitions *state)
{
	...
	for (i = 0; i < le32_to_cpu(gpt->num_partition_entries) && i < state->limit-1; i++) {
		...
		if (!is_pte_valid(&ptes[i], last_lba(state->bdev)) || i >= 11)
			continue;
	...
	for (j = 0; j < (sizeof(dni_custom_part) / sizeof(struct dni_partition)); j++){
		struct partition_meta_info *info_dni;
		u64 start_r, size_r;

		start_r = dni_custom_part[j].start_r;
		size_r = dni_custom_part[j].size_r;

		if (!is_pte_valid(&ptes[j], last_lba(state->bdev)))
			continue;

		put_partition(state, k+1, start_r * ssz, size_r * ssz);

		info_dni = &state->parts[k + 1].info;
		strcpy(info_dni->volname, dni_custom_part[j].name);

		pr_notice("#: vol name is %s, start is 0x%llx, size is 0x%llx, ssz is 0x%x\n", info_dni->volname, start_r, size_r, ssz);
		if (ROOT_DEV == 0 && !strcmp(info_dni->volname, "rootfs") &&
			config_enabled(CONFIG_MTD_ROOTFS_ROOT_DEV)) {
