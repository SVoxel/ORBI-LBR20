...
if [ "$1" ]; then
    ...
    cp -t "$work_dir"/IPQ4019.ILQ.4.0/common/build/ipq/ $@ || exit 1
    echo >&2 "done"
fi

echo >&2 -n "Generating single images ... "
(
    cd "$work_dir"/IPQ4019.ILQ.4.0/common/build/ && python update_common_info.py
)

if [ "$output_dir" ]; then
    echo >&2 -n "Copying single images to $output_dir ... "
    cp -t "$output_dir" "$work_dir"/IPQ4019.ILQ.4.0/common/build/bin/*.img
    echo >&2 "done"
fi
