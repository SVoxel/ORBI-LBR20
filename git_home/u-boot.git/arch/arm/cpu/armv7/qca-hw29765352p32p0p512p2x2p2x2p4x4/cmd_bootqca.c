...
static int do_boot_unsignedimg2(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	u8 board_data[CRASHDUMP_SWITCH_LENGTH + 1];
	get_board_data_in_art(board_data);
if (simple_strtol(board_data, (char **)NULL, 16) == 0x1){
	ret = scm_call(SCM_SVC_BOOT, SCM_SVC_RD, NULL,
			0, (void *)&val, sizeof(val));
	/* check if we are in download mode */
	if (val == DLOAD_MAGIC_COOKIE) {
...
static int do_boot_check_crashdump(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (argc == 2 && strncmp(argv[1], "debug", 5) == 0)
		debug = 1;
	u8 board_data[CRASHDUMP_SWITCH_LENGTH + 1];
	get_board_data(CRASHDUMP_SWITCH_OFFSET, CRASHDUMP_SWITCH_LENGTH, board_data);
	switch_val = simple_strtol(board_data, (char **)NULL, 16); 
                            ...
                            char * const argv[])
{
	...
	if (argc ==2 && (val==1 || val==0)){
		printf("argc is %d , val is %d \n",argc ,val);
		memcpy(board_data, argv[1], CRASHDUMP_SWITCH_LENGTH);
		set_board_data(CRASHDUMP_SWITCH_OFFSET, CRASHDUMP_SWITCH_LENGTH, board_data);
                             ...
                             char * const argv[])
{
	...
	get_board_data(CRASHDUMP_SWITCH_OFFSET, CRASHDUMP_SWITCH_LENGTH, board_data);
	val = simple_strtol(board_data, (char **)NULL, 16);
	//get_board_data_in_art(board_data);
	if(val == 0x1)
